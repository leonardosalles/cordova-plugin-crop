    ...
    public void onActivityResult(int requestCode, int resultCode, Intent intent) {
        if (requestCode == Crop.REQUEST_CROP) {
            if (resultCode == Activity.RESULT_OK) {
                final Uri imageUri = Crop.getOutput(intent);

                cordova.getActivity().runOnUiThread(new Runnable() {
                    public void run() {
                        cropPlugin.callbackContext.success("file://" + imageUri.getPath() + "?" + System.currentTimeMillis());
                        cropPlugin.callbackContext = null;
                    }
                });
                ...
                cordova.getActivity().runOnUiThread(new Runnable() {
                    public void run() {
                        try {
                            ...
                            err.put("code", String.valueOf(Crop.RESULT_ERROR));
                            cropPlugin.callbackContext.error(err);
                            cropPlugin.callbackContext = null;
